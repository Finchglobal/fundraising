...
async function fix() {
  ...
  if (adminUser) {
    // 1. Ensure password is set correctly just in case
    await supabase.auth.admin.updateUserById(adminUser.id, { password: '72099Sh@', email_confirm: true });
    
    // 2. Force profile role to super_admin
